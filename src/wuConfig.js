  ...
  wu.get(configFile, content => {
    ...
    if (e.extAppid)
      wu.save(
        ...
        JSON.stringify({ extEnable: true, extAppid: e.extAppid, ext: e.ext }, null, 4)
      )
        ...
        digestsEvent.add(() => {
          for (let e of app.tabBar.list) {
            ...
            if (e.selectedIconData) {
              let hash = crypto
                .createHash('MD5')
                .update(e.selectedIconData, 'base64')
                .digest()
              for (let [buf, name] of digests)
                if (hash.equals(buf)) {
