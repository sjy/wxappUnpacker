...
function doWxss(dir, cb) {
  ...
  function cssRebuild(data) {
    ...
    function makeup(data) {
      ...
      if (isPure && actualPure[data] != cssFile) {
        if (actualPure[data])
          return '@import "' + wu.changeExt(wu.toDir(actualPure[data], cssFile), '.wxss') + '";\n'
        else {
      ...
      if (name != frameFile) {
        wu.get(name, code => {
          ...
          if (code.indexOf('setCssToHead') > -1)
            runList[name] = code.slice(code.indexOf('setCssToHead'))
        })
      }
  }
          ...
          if (son.type == 'Declaration') {
            if (list[son.property]) {
              ...
              if (x.value.type == 'Raw' && x.value.value.startsWith('progid:DXImageTransform')) {
                ...
                let xValue =
                    x.value.children && x.value.children.head && x.value.children.head.data.name,
                  yValue =
                    y.value.children && y.value.children.head && y.value.children.head.data.name
                if (xValue && yValue)
  ...
  wu.scanDirByExt(dir, '.html', files => {
    ...
    wu.get(frameFile, code => {
      ...
      let mainCode = oriCode.slice(
        oriCode.indexOf('setCssToHead'),
        oriCode.lastIndexOf(';var __pageFrameEndTime__')
      )
        ...
        for (let id in pureData)
          if (!actualPure[id]) {
            ...
            if (importCnt[id] <= 1) {
              ...
              console.log(
                'Cannot find pure import for _C[' + id + '], force to save it in (' + newFile + ').'
              )
